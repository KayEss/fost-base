...
FSL_TEST_FUNCTION(message) {
    felspar::u8string test_level = "test-level";
    ...
    felspar::u8string test_level2 = "test-level2";
...
//     fostlib::log::warning(c_module, "Example warning message -- please
//     ignore"); fostlib::log::error(c_module, "Example error message -- please
//     ignore"); fostlib::log::critical(
//             c_module, "Example critical message -- please ignore");
...
//     using namespace fostlib::log;
//     scoped_sink<capture_copy> cc;
//     debug(c_module)("key", "value")("second-key", "part-a", true)(
//             "second-key", "part-b", false)("third-key", fostlib::json());
//     debug(c_module)("goodbye", "country")("hello", "nightclub");
//     fostlib::json data = cc();
//
//     FSL_CHECK(data[0].has_key("module"));
//     FSL_CHECK_EQ(data[0]["body"]["key"], fostlib::json("value"));
//     FSL_CHECK_EQ(data[0]["body"]["second-key"]["part-a"],
//     fostlib::json(true));
//     FSL_CHECK_EQ(data[0]["body"]["second-key"]["part-b"],
//     fostlib::json(false));
...
//     scoped_sink<capture_copy> cc;
//     FSL_CHECK_EXCEPTION(
//             global_sink_configuration sinks(conf), fostlib::exceptions::null
//             &);
...
//
//     fostlib::log::global_sink_configuration gsc(config);
//     fostlib::log::info(c_module, "Sending through to the global
//     configuration");
//
//     fostlib::log::flush();
//     {
...
//     FSL_CHECK_EQ(
//             data[1]["body"],
//             fostlib::json("Sending through to the global configuration"));
// }
...
namespace {
    ...
    class count_log_messages {
      public:
        count_log_messages(fostlib::json const &) {}
        bool operator()(fostlib::log::message const &) const {
            ++g_messages_seen;
            return false;
        }
    };
...
//     fostlib::log::global_sink_configuration gsc(config);
//
//     for (std::size_t c(0); c < 1000; ++c) { fostlib::log::debug(c_module, c);
//     } fostlib::log::flush();
//
