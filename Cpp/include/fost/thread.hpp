...
namespace fostlib {
    ...
    namespace detail {
        /// The class that actually stores the future. Always pass to
        /// fostlib::future rather than use it directly
        template<typename R>
        class future_result;
    }
    ...
    class FOST_CORE_DECLSPEC worker {
      ...
      public:
        ...
        template<typename R>
        std::shared_ptr<detail::future_result<R>>
                operator()(std::function<R(void)> f) {
            return run<R>(f);
        }

        template<typename R, typename F>
        std::shared_ptr<detail::future_result<R>> run(F f) {
            std::shared_ptr<detail::future_result<R>> future{
                    new detail::future_result<R>};
            queue(future,
                  typename detail::future_result<R>::function(future, f));
            return future;
        }
      ...
      private:
        void
                queue(std::shared_ptr<detail::future_result<void>> j,
 ...
 */
        ...
        class future_result : public future_result<void> {
          private:
            struct function {
                function(
                        std::shared_ptr<future_result<R>> j,
    ...
    template<>
    inline std::shared_ptr<detail::future_result<void>>
            worker::run(std::function<void(void)> f) {
    ...
    class future {
      public:
        ...
        future(std::shared_ptr<detail::future_result<R>> r) : m_result(r) {}
        ...
        void assert_valid() const {
            if (!m_result.get())
                throw exceptions::null(
                        "The result/future has not been initialised");
        }
        std::shared_ptr<detail::future_result<R>> m_result;

        template<typename O>
        friend class in_process;
    };
    ...
    class future<void> {
      ...
      public:
        ...
        future() {}
        /// Construct a future that will get a result
        future(std::shared_ptr<detail::future_result<void>> r) : m_result(r) {}
    ...
    /// Wraps fostlib::worker to give a simpler synchronous and asynchronous
    /// processing model
    template<typename O>
    class in_process : private worker {
      private:
        ...
        struct functor {
            ...
            O &m_o;
            std::function<B(O &)> m_f;
        };

      public:
        /// Construct the in_process from anything convertible to a function
        /// returning O*
        template<typename F>
        in_process(F c)
        : object((worker::operator()(std::function<O *(void)>(c)))->result()) {}
        explicit in_process(O *o) : object(o) {}
