...
#define FSL_MAIN_PRIVATE_INTERPRETERMAIN() \
    int main_body( \
            const fostlib::loaded_settings &, fostlib::ostream &, \
            fostlib::arguments &);
...
#define FSL_MAIN_PRIVATE_CALLER(exe_name, banner_text) \
    int __cdecl wmain(int argc, wchar_t *argv[], wchar_t *envp[]) { \
        ...
        return fostlib::main_exec( \
                config_settings, std::wcout, args, main_body); \
    } \
    int main_body
...
#define FSL_MAIN_PRIVATE_CALLER(exe_name, banner_text) \
    int main(int argc, char *argv[]) { \
        ...
        return fostlib::main_exec( \
                config_settings, std::cout, args, main_body); \
    } \
    int main_body
...
/**  \brief A muliti-platform Unicode aware main that handles argument parsing
    ...
    This version of main is suitable for most programs that simply need to take
   settings and start some task for the user.

    This version will load the INI file for the program and the -i command line
   switch is handled.
*/
...
/** \brief A multi-platform Unicode aware main for programming language
   interpreters
    ...
    NB: The INI file for the program is not automatically loaded by this
   version. The -i command line switch is not handled either.
*/
